...
class ColorHistogram {
    ...
    count_Pixels (pixels) {
        ...
        const nPixels = pixels.data.length / 4

        // percorrer todos os pixels da imagem
        for(let index = 0; index < nPixels; index++){

            //informação (r,g,b) do pixel
            let pR = pixels.data[(index*4) + 0];
            let pG = pixels.data[(index*4) + 1];
            let pB = pixels.data[(index*4) + 2];

            // calcular a distancia de Manhattan entre o pixel e cada uma das cores
            for(let i = 0; i < this.redColor.length; i++){
                let distR = Math.abs(this.redColor[i] - pR);
                let distG = Math.abs(this.greenColor[i] - pG);
                let distB = Math.abs(this.blueColor[i] - pB);
                let manDist = distR + distG + distB;
                
                if (manDist < this.thold1 && distR < this.thold2 && distG < this.thold2 && distB < this.thold2) hist[i] += 1;
            }
        }
